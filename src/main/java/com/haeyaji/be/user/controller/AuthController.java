...
public class AuthController {
    ...
    public ResponseEntity<Void> reissue(HttpServletRequest request, HttpServletResponse response) {
        ...
        // 3) jwtTokenProvider.getUserId(refreshToken)로 userId 추출
        UUID userId = jwtTokenProvider.getUserId(refreshToken);

        // 4) refreshTokenRepository.findByUserId(userId)로 Redis에 저장된 값과 대조
        // 없거나 불일치하면 401 — 이미 로그아웃됐거나 탈취/재사용된 토큰일 수 있음
        String existingRefreshToken = refreshTokenRepository.findByUserId(userId)
        ...
        // 5) userRepository.findById(userId)로 최신 role 조회
        ...
        String newRefreshToken = jwtTokenProvider.createRefreshToken(userId);    // Refresh Token Rotation (RTR)

        // 7) refreshTokenRepository.save()로 새 refreshToken을 Redis에 회전 저장 (재사용 공격 방지)
        // redisTemplate: 같은 key로 save하면 덮어쓰기됨
        refreshTokenRepository.save(userId, newRefreshToken, Duration.ofMillis(jwtTokenProvider.getRefreshTokenValidityMs()));
